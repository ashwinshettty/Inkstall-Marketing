...
const LastYear = ({ leads = [] }) => {
  // Filter leads from the last 365 days and group by status
  const recentLeads = filterLeadsByDateRange(leads, 365);
  const leadsByStatus = groupLeadsByStatus(recentLeads);
  const dateRange = getDateRangeText(365);
  const hasLeads = recentLeads.length > 0;

  return (
    <div className="mt-4">
      <h2 className="text-xl font-semibold mb-2">Last Year Analytics</h2>
      ...
      {hasLeads ? (
        ...
        <div className="bg-gray-800 p-6 rounded-lg text-center">
          <p className="text-gray-400">No leads were active in the last year.</p>
        </div>
      )}
    </div>
  );
