...
const LastQuarter = ({ leads = [] }) => {
  // Filter leads from the last 90 days and group by status
  const recentLeads = filterLeadsByDateRange(leads, 90);
  const leadsByStatus = groupLeadsByStatus(recentLeads);
  const dateRange = getDateRangeText(90);
  const hasLeads = recentLeads.length > 0;

  return (
    <div className="mt-4">
      <h2 className="text-xl font-semibold mb-2">Last Quarter Analytics</h2>
      ...
      {hasLeads ? (
        ...
        <div className="bg-gray-800 p-6 rounded-lg text-center">
          <p className="text-gray-400">No leads were active in the last quarter.</p>
        </div>
      )}
    </div>
  );
